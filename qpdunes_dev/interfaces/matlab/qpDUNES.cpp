...
//	#endif
		 ...
		 ( strcmp( typeString,"SOLVE" ) == 0 ) )
	{
        ...
        /* HARDCODED REALLY NASTY CODE */
        if (nlhs > 0) {
            nIter = qpDataGlobal->log.numIter-1;
            plhs[nlhs-1] = mxCreateDoubleMatrix(nIter, 6, mxREAL);
            double* timingsPtr = mxGetPr( plhs[nlhs-1] );
            for( i = 0; i < nIter; i++ ) {
                timingsPtr[i] = qpDataGlobal->log.itLog[i+1].tIt;
                timingsPtr[nIter+i] = qpDataGlobal->log.itLog[i+1].tNwtnSetup;
                timingsPtr[2*nIter+i] = qpDataGlobal->log.itLog[i+1].tNwtnFactor;
                timingsPtr[3*nIter+i] = qpDataGlobal->log.itLog[i+1].tNwtnSolve;
                timingsPtr[4*nIter+i] = qpDataGlobal->log.itLog[i+1].tQP;
                timingsPtr[5*nIter+i] = qpDataGlobal->log.itLog[i+1].tLineSearch;
            }
        }
        /*qpData->log.itLog[(*itCntr)].tIt = tItEnd - tItStart;
        qpData->log.itLog[(*itCntr)].tNwtnSetup = tNwtnSetupEnd - tNwtnSetupStart;
        qpData->log.itLog[(*itCntr)].tNwtnSolve = tNwtnSolveEnd - tNwtnSolveStart;
        qpData->log.itLog[(*itCntr)].tQP = tQpEnd - tQpStart;
        qpData->log.itLog[(*itCntr)].tLineSearch = tLineSearchEnd - tLineSearchStart;*/
