...
% 	cmd = [ 'mex -v CC="gcc" CXX="gcc" LD="gcc" COPTIMFLAGS="$COPTIMFLAGS -O3 -fopenmp" -lgomp  -output ', MEXOBJS{i}, ' ', DEBUGFLAGS, CPPFLAGS, [MEXOBJS{i},'.cpp ',QPDUNES_OBJECTS] ]
    cmd = [ 'mex -v CC="clang" CXX="clang" LD="clang" CXXLIBS="$CXXLIBS -lstdc++" CLIBS="$CLIBS -lstdc++" COPTIMFLAGS="$COPTIMFLAGS -O3" -output ', MEXOBJS{i}, ' ', DEBUGFLAGS, CPPFLAGS, [MEXOBJS{i},'.cpp ',QPOASES_OBJECTS QPDUNES_OBJECTS] ]
